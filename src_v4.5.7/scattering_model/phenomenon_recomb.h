...
public:
		...
		: tau(param->tau_phenom_recomb), nk_glob(nk_glob), nb_dm(nb_dm), nv_dm(nv_dm)
	{
		...
		if (ionode) printf("==================================================\n");
		if (ionode) printf("==================================================\n");
		f_eq = alloc_real_array(nk_glob, nb_dm);
		dm = alloc_array(nk_glob, nb_dm*nb_dm);
		ddmdt_phenom = alloc_array(nk_glob, nb_dm*nb_dm);
	}

    void evolve(complex **dm, double **f_eq, complex **ddmdt_phenom);
};

void phenom_recomb::evolve(complex **dm, double **f_eq, complex **ddmdt_phenom)
{
        ...
        for (int i = 0; i < nb_dm; i++)
        {
            for (int j = 0; j < nb_dm; j++)
            {
                ...
                if (i != j)
                    ...
                    ddmdt_phenom[ik_glob][i*nb_dm + j] += -(dm[ik_glob][i*nb_dm + j] - f_eq[ik_glob][i]) / tau;
            }
        }
    }
}