...
public:
		...
		: latt(latt), param(param), elec(elec), elight(elight), eph(eph)
	{
		...
		if (!(pmp.active() && (elight->pumpMode == "lindblad" || elight->pumpMode == "coherent")) && alg.scatt_enable){
	...
	bool update_eimp_model_inside(double t){
		bool update = pmp.active() && elight->during_pump(t) && param->freq_update_eimp_model < 0 && !alg.linearize;
	...
	bool update_eimp_model_outside(double t, int it){
		bool update = pmp.active() && elight->during_pump(t) && param->freq_update_eimp_model > 0 && (it - 1) % param->freq_update_eimp_model == 0 && !alg.linearize;
		if (!update) return false;
		for (int iD = 0; iD < eip.ni.size(); iD++)
			if (eph->eimp[iD]->eimp_model != nullptr && update) return true;
		return false;
